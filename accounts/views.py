...
def signout(request):
    ...
    try:
        # attempt to revoke the Supabase session if we have a token
        token = request.session.get('supabase_jwt')
        if token:
            try:
                # preferred modern client API
                supabase.auth.sign_out()
            except Exception:
                # fallback to older api method if available
                try:
        ...
        # remove any stored supabase token and Django session
        request.session.pop('supabase_jwt', None)
        logout(request)
